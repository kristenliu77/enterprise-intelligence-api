...
export function MetricCard({ title, value, change, tooltip, trend, suffix }: MetricCardProps) {
  const option: EChartsOption = {
    ...
    series: [{ type: "line", smooth: true, symbol: "none", data: trend, lineStyle: { color: "#1677FF", width: 2 }, areaStyle: { color: "rgba(22,119,255,0.12)" } }]
  };
  return (
    <Card className="metric-card">
      <Space direction="vertical" size={8} style={{ width: "100%" }}>
        ...
        <Statistic value={value} suffix={suffix} valueStyle={{ color: "#1F2937", fontWeight: 800 }} />
        <Space>
          {change >= 0 ? <ArrowUpOutlined style={{ color: "#0E8A78" }} /> : <ArrowDownOutlined style={{ color: "#D64545" }} />}
          <Typography.Text type={change >= 0 ? "success" : "danger"}>{Math.abs(change)}%</Typography.Text>
